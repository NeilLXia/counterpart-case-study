...
def limit_retention_df():
    return pd.DataFrame({
        "limit_retention_amount": [10_000, 100_000, 500_000],
...
class TestValidateColumns:
    def test_missing_column_raises(self, limit_retention_df, industry_factor_df):
        bad_df = pd.DataFrame({"asset_size": [1_000_000]})  # missing base_rate
...
class TestGetBasePremium:
    def test_exact_match_lower_bound(self, rating_tables):
        assert rating_tables.get_base_premium(1_000_000) == 1819

    def test_exact_match_upper_bound(self, rating_tables):
        assert rating_tables.get_base_premium(5_000_000) == 3619

    def test_interpolates_between_two_rows(self, rating_tables):
        # 3000000 is 20% of the way between 2500000 (3966) and 5_000_000 (3619) → 3896.6
        result = rating_tables.get_base_premium(3_000_000)
...
class TestGetLimitFactor:
    def test_exact_match_lower_bound(self, rating_tables):
        assert rating_tables.get_limit_factor(10_000) == pytest.approx(-0.231)

    def test_exact_match_upper_bound(self, rating_tables):
        assert rating_tables.get_limit_factor(500_000) == pytest.approx(0.756)
    ...
    def test_above_maximum_raises(self, rating_tables):
        with pytest.raises(ValueError, match="outside the table range"):
            rating_tables.get_limit_factor(9_999_999)


class TestGetRetentionFactor:
    def test_exact_match(self, rating_tables):
        assert rating_tables.get_retention_factor(
            100_000) == pytest.approx(0.350)
    ...
    def test_uses_retention_factors_table_not_limit_factors(self, industry_factor_df):
        ...
        assert tables.get_retention_factor(100_000) == pytest.approx(10.0)
